    ...
    <form action="{{ route ('admin.posts.update', ['posts' => $post->id]) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="category_id" id="category_id">
                ...
                @foreach ($categories as $category)
                   <option value="{{ $category->id }}" {{ $post->category && (old('category_id', $post->category->id) == $category->id) ? 'selcted' : '' }}>{{ $category->name }}</option> 
                @endforeach
            </select>
        </div>
        ...
        <div class="">
            ...
            <div class="form-check">
                ...
                <label class="form-check-label" for="tag-{{ $tag->id }}" checked>
                  {{ $tag->name}}
                </label>
              </div>
            @endforeach
